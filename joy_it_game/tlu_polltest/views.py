...
import logging
import smtplib
from tlu_polltest.models import Level, getGameState, setGameState
...
class IndexView(generic.TemplateView):
    ...
    '''
    template_name = 'tlu_polltest/index.html'
...
class DemoView(generic.TemplateView):
    ...
    '''
    template_name = 'tlu_polltest/demo.html'
...
def LevelOverview(request):
    ...
    if request.method=='GET':
        ...
        level=Level00(request.user.id)
        level.startup()  
        template_name = 'tlu_polltest/level_overview.html'
        ...
        if (cur_level != None) and (cur_level.result == Level.PASSED):
            ...
            if game.completed(next_level_num):
                ...
                path=reverse('tlu_polltest:retry', kwargs={'level_id':next_level_num})
        if len(path) == 0:
            path=reverse('tlu_polltest:level', kwargs={'level_id':next_level_num})
        last_played=None
        if (cur_level != None):
            last_played=cur_level.level_start
        if last_played == None:
            last_played=_("Not yet played")
            path='/tlu_polltest/restart'
...
def restart(request):
    ...
    game.restart()
    return HttpResponseRedirect(reverse('tlu_polltest:level', kwargs={'level_id':1}))
...
def renderLevel(request, level_id):
    ...
    button_url='/tlu_polltest/start/'+str(level_id)
...
def level_update(request):
    ...
    if request.method=='GET':
        ...
        button_url='/tlu_polltest/start/'+str(current_level_num)
        info=status.msg
        if (info==None):
        ...
        if game.is_running(): 
            ...
            button_url='/tlu_polltest/abort/'+str(current_level_num)
        else:
            res=game.result(0.5)
            ...
            if res==Level.PASSED:
                ...
                button_url='/tlu_polltest/level/'+str(current_level_num+1)
                if game.endReached():
                    button_text=_("Restart?")
                    button_url='/tlu_polltest/restart'
            elif res==Level.TIMEOUT:
                #timeout reached
                button_text=_("Retry")
                button_url='/tlu_polltest/retry/'+str(current_level_num)               
            else:
                #failure somehow
...
class LoginView(generic.ListView):    
    ...
    * to be removed *
    '''
    template_name = 'tlu_polltest/login.html'
...
class DetailView(generic.DetailView):
    ...
    * to be removed *
    '''
    model = Question
    template_name = 'tlu_polltest/level00.html'
...
class ResultsView(generic.DetailView):
    ...
    * to be removed *
    '''
    model = Question
    template_name = 'tlu_polltest/level01.html'
...
def vote(request, question_id):
    ...
    * to be removed *
    ...
    try:
        ...
        return render(request, 'tlu_polltest/level00.html', {
            'question': question,
        ...
        return HttpResponseRedirect(reverse('tlu_polltest:results', args=(question.id,)))
...
def settings(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('tlu_polltest:emailsettingscheck'))
        else:
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if bool(form_dict.get('emailSuccess')):
                return HttpResponseRedirect(reverse('tlu_polltest:index'))
            try:
